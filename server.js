...
app.get("/api/volgend-offertenummer", async (req, res) => {
  try {
    const teller = await storage.leesToeller();
    const volgend = (teller.offerte || 0) + 1;
    const nummer = `${config.offertenummerVoorvoegsel}${config.startjaar}-${String(volgend).padStart(4, "0")}`;
    res.json({ nummer });
  } catch (e) {
    res.status(500).json({ fout: "Kon nummer niet ophalen" });
  }
});

app.get("/api/check-offertenummer/:nummer", async (req, res) => {
  try {
    const bestaatAl = await storage.offertenummerBestaatAl(req.params.nummer.trim());
    res.json({ bestaatAl });
  } catch (e) {
    res.status(500).json({ fout: "Kon nummer niet controleren" });
  }
});
...
app.post("/offerte/nieuw", async (req, res) => {
  try {
    const body = req.body;
    const klant = {
    ...
    let regels = [];
    try {
      regels = JSON.parse(body.werkRegelsJson || "[]")
        .map((r) => ({
          omschrijving: (r.omschrijving || "").trim(),
          aantal: Number(r.aantal) || 0,
          eenheid: r.eenheid || "stuk",
          prijsPerEenheid: Number(r.prijsPerEenheid) || 0,
        }))
    ...
    if (handmatigNummer) {
      if (await storage.offertenummerBestaatAl(handmatigNummer)) {
        return res.redirect(`/?fout=duplicate&nummer=${encodeURIComponent(handmatigNummer)}`);
      }
      offertenummer = handmatigNummer;
    } else {
      const volgnummer = await storage.volgendNummer("offerte");
    ...
    const record = {
      id: crypto.randomUUID(), status: "offerte", offertenummer,
      aangemaaktOp: vandaag.toISOString(), geldigTot: geldigTot.toISOString(),
      klant, klus: { opmerkingen }, berekening,
      factuurnummer: null, factuurdatum: null, vervaldatum: null,
    };

    await storage.voegOfferteToe(record);
    res.redirect(`/offerte/${record.id}`);
  } catch (e) {
    console.error("Fout bij aanmaken offerte:", e);
    res.status(500).send("Er ging iets mis bij het opslaan. Probeer opnieuw.");
  }
});

app.get("/offerte/:id", async (req, res) => {
  try {
    const record = await storage.vindOfferteOpId(req.params.id);
    ...
    res.status(500).send("Kon offerte niet laden.");
  }
});

app.post("/offerte/:id/akkoord", async (req, res) => {
  try {
    const record = await storage.vindOfferteOpId(req.params.id);
    if (!record) return res.status(404).send("Offerte niet gevonden.");
    if (record.status !== "factuur") {
      ...
      const volgnummer = await storage.volgendNummer("factuur");
      const factuurnummer = `${config.factuurnummerVoorvoegsel}${config.startjaar}-${String(volgnummer).padStart(4, "0")}`;
      await storage.updateOfferte(record.id, {
        status: "factuur", factuurnummer,
    ...
    console.error("Fout bij akkoord:", e);
    res.status(500).send("Kon factuur niet aanmaken.");
  }
});
...
app.get("/overzicht", async (req, res) => {
  try {
    const lijst = (await storage.leesAlleOffertes()).slice().reverse();
    ...
    console.error("Fout bij overzicht:", e);
    res.status(500).send("Kon overzicht niet laden.");
  }
});
...
async function start() {
  try {
    await storage.initialiseerDatabase();
  } catch (e) {
    console.error("Database initialisatie mislukt (mogelijk geen DATABASE_URL ingesteld):", e.message);
  }
  app.listen(PORT, () => {
    console.log("==========================================");
    console.log(` ${config.bedrijf.naam} — Server draait op poort ${PORT}`);
    console.log(` Database: ${process.env.DATABASE_URL ? "PostgreSQL ✓" : "JSON bestanden (lokaal)"}`);
    console.log("==========================================");
  });
}
